...
void ListClubs(std::list<Club> clubs) {  // Funcion para Mostrar Clubes
  ...
  while (!tempClubs.empty()) {
    ...
    std::cout << "Miembros del club actuales: " << tempClub.members.size()
              << std::endl;
    std::cout << std::endl;
    tempClubs.pop_front();
  }
...
void TotalMembers(std::list<Club> clubs) {  // Funcion para mostrar el total de
                                            // miembros de un club
  ...
  std::cout << "La cantidad de miembros del club: " << clubFound.name << "es "
            << clubFound.members.size() << std::endl;
}

void RegisterMember(
    std::list<Club>& clubs) {  // Funcion para registrar un miembro en un club
  if (clubs.empty()) {
    std::cout << "La Lista de Clubes esta vacia" << std::endl;
  }
...
void ListMemebers(
    std::list<Club> clubs) {  // Funcion para mostrar miembros de un club
  Club clubFound = FindClub(clubs);
  ...
  while (!members.empty()) {
    ...
    std::cout << "Nombre: " << tempMember.first_name
              << ", Apellido: " << tempMember.last_name
              << ", Edad: " << tempMember.age << std::endl;
    members.pop_front();
  }
}
